...
//                  {alert.userId?.fullname} ({alert.userId?.email})
...
const PoliceAlertsDashboard = () => {
  ...
  const fetchAlerts = async () => {
    ...
    setError(null);
    try {
      const token = localStorage.getItem("token"); // Ensure token is stored with key "token"
      const response = await fetch("http://localhost:3000/police/alert", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      const data = JSON.parse(text); // Parse JSON manually since we already used text()
      setAlerts(data);
    } catch (err) {
      console.error(err);
      setError('An error occurred while fetching alerts. Please try again.');
...
<div className="flex items-center space-x-3 mb-4">
  ...
  <span className="text-lg font-medium text-blue-300">
    {alert.userId
      ? `${alert.userId.fullname || alert.userId} (${alert.userId.email || "No Email"})`
      : "Unknown User"}
  </span>
</div>
