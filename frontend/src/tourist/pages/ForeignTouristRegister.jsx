...
import { FiUser, FiGlobe, FiCalendar, FiLock, FiMapPin, FiCheckCircle, FiAlertCircle } from "react-icons/fi";
...
export default function ForeignTouristRegister() {
    ...
    const [formData, setFormData] = useState({
        fullname: '', gender: '', date_of_birth: '', nationality: '', passportNumber: '', visaNumber: '',
        email: '', phoneNumber: '', arrivalDate: '', departureDate: '', flightNumber: '', destination: '',
        password: '', confirmPassword: '', lat: null, lng: null,
        smartTouristId: `ST-FOR-${Date.now()}`
    });
    ...
    return (
        <div className="bg-slate-100 min-h-screen font-sans flex items-center justify-center p-4">
            <div className="w-full max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 bg-white rounded-2xl shadow-2xl overflow-hidden">
                ...
                {/* --- Right Panel: Form --- */}
                <div className="p-8">
                    <h1 className="text-3xl font-bold text-slate-800">Tourist Registration</h1>
                    <p className="text-slate-500 mt-1">Create your secure account in minutes.</p>
                    ...
                    <form onSubmit={handleSubmit}>
                        {currentStep === 1 && (
                            <section className="space-y-4">
                                <h2 className="font-semibold text-lg text-slate-700 flex items-center gap-2"><FiUser /> Personal Information</h2>
                                <InputField name="fullname" type="text" placeholder="Full Name" value={formData.fullname} onChange={handleChange} />
                                <InputField name="passportNumber" type="text" placeholder="Passport Number" value={formData.passportNumber} onChange={handleChange} />
                                <InputField name="visaNumber" type="text" placeholder="Visa Number" value={formData.visaNumber} onChange={handleChange} />
                                <InputField name="nationality" type="text" placeholder="Nationality" value={formData.nationality} onChange={handleChange} />
                        ...
                        {currentStep === 2 && (
                            <section className="space-y-4">
                                ...
                                <InputField name="departureDate" type="date" placeholder="Departure Date" value={formData.departureDate} onChange={handleChange} />
                                <InputField name="flightNumber" type="text" placeholder="Flight/Train Number" value={formData.flightNumber} onChange={handleChange} />
                                <InputField name="destination" type="text" placeholder="Primary Hotel/Destination" value={formData.destination} onChange={handleChange} />
