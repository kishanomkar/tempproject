...
import Police, { ReqAlert } from '../models/police.model.js';
...
// import { ReqAlert } from '../models/police.model.js';
...
export const sendAlert = async (req, res) => {
  try {
    ...
    const alert = new ReqAlert({ type, message });
    await alert.save();
...
export const getAlerts = async (req, res) => {
  try {
    const alerts = await ReqAlert.find().sort({ time: -1 });
    res.json(alerts);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
